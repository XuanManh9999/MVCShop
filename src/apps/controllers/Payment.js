...
const payment = async (req, res) => {
  ...
  try {
    ...
    if (result.data && result.data.payUrl) {
      ...
      return res
        .status(500)
        .json({ message: "Server error: No payUrl from MoMo" });
    }
  } catch (error) {
    ...
    if (newOrder && newOrder._id) {
      // Kiểm tra nếu newOrder đã được tạo thành công
      console.log(
        "Đã xảy ra lỗi sau khi tạo đơn hàng. Cập nhật trạng thái thành 'failed' cho ID:",
        newOrder._id
      );
      await orderModel.findByIdAndUpdate(newOrder._id, { status: "failed" });
    }
...
const callback = async (req, res) => {
  ...
  if (!order) {
    console.error(
      `Không tìm thấy đơn hàng với ID: ${momoOrderId} trong database.`
    );
  ...
  if (resultCode === "0") {
    ...
    for (const item of order.items) {
      await productModel.updateOne(
        { _id: item.prd_id },
        { $inc: { stock: -item.prd_qty } }
      );
    }
    console.log("Đã cập nhật tồn kho cho đơn hàng:", order._id);

    // --- Tạo warranty records cho đơn hàng ---
    const warrantyController = require("./Warranty");
    try {
      await warrantyController.createWarrantyRecords(order, req);
      console.log("Đã tạo warranty records cho đơn hàng:", order._id);
    } catch (error) {
      console.error("Lỗi tạo warranty records:", error);
    }
    ...
    console.log(
      `Thanh toán thất bại cho đơn hàng ${order._id} với resultCode: ${resultCode}.`
    );
