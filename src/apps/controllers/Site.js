...
const postSignUp = async (req, res) => {
  const { full_name, email, phone, address, password, confirmPassword } =
    req.body;
...
const historyOrder = async (req, res) => {
  ...
  if (email) {
    ...
    if (customer) {
      // nếu có người dùng đó bắt đầu lọc sản phẩm đã mua
      const page = parseInt(req.query.page) || 1;
      const limit = 5;
      const skip = (page - 1) * limit; // Chuẩn hóa công thức skip
      const totalRows = await orderModel.countDocuments({ email }); // Fix: chỉ đếm orders của customer hiện tại
      ...
      res.render("site/historyOrder", {
        // chuyển hết thông tin model và các lib tự tạo sang ví dụ như vndprice..
        items,
        vndPrice,
        email,
        orders,
...
const order = async (req, res) => {
  ...
  // 🔻 Tạo warranty records cho mỗi item trong đơn hàng
  try {
    await warrantyController.createWarrantyRecords(newOrder, req);
  } catch (error) {
